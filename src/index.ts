...
async function run() {
  try {
    ...
    // ⚡ Bolt Optimization: Batch LLM API requests to improve performance
    // Processing files in parallel speeds up the review significantly.
    // We use a batch size of 3 to balance speed and avoid HTTP 429 rate limits.
    const BATCH_SIZE = 3;
