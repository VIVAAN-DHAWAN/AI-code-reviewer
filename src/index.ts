...
async function run() {
  try {
    ...
    // ⚡ Bolt: Batch API requests to process files concurrently (batch size = 3).
    // Reduces overall review time by preventing sequential bottleneck while
    // avoiding HTTP 429 'Too Many Requests' rate limits from AI providers.
    const BATCH_SIZE = 3;
    const filesToReview = files.filter(file => file.diff.trim());
