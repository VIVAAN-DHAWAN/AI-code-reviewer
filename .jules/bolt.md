...
## 2024-05-24 - Parallelizing GitHub API Calls

**Learning:** The application was awaiting GitHub API calls sequentially (`getPRDetails` then `getPRDiff`, and `postReviewComments` then `postSummary`), leading to unnecessary network wait time.
**Action:** Use `Promise.all` to execute independent asynchronous API calls concurrently, significantly improving the overall speed of the action.
